...
public class ApuestasUI {
    ...
    public void menuApuestas() {
        ...
        while (true) {
            ...
            System.out.println("3. Reclamar resultados");
            ...
            switch (input) {
                ...
                case 3 -> {
                   System.out.println("Escribe el numero de cedula del participante a consultar");
                    String cedula = scanner.nextLine();
                    Participante participante = ParticipantesController.traerParticipantePorCedula(cedula);
                    if (participante == null) {
                        System.out.println("El participante no existe!");
                        ...
                        ArrayList<Apuesta> apuestas = ApuestasController.apuestasDeParticipante(participante);
                        for(int i = 0; i < apuestas.size(); i++){
                            Apuesta apuesta = apuestas.get(i);
                            System.out.println((i + 1) + ". " + apuesta.getCompetencia().getLugar() + "| Burro: " + apuesta.getBurro().getNombre() + " | Ganador: " + apuesta.getCompetencia().getGanador().getId());
                        }
                        int eleccionApuesta = Integer.parseInt(scanner.nextLine());
                        Apuesta apuestaSeleccionada = apuestas.get(eleccionApuesta - 1);
                        if(apuestaSeleccionada.getBurro().getId() == apuestaSeleccionada.getCompetencia().getGanador().getId()){
                            System.out.println("Haz ganado la apuesta = saldo actual x 2");
                            participante.setSaldoDisponible(participante.getSaldoDisponible() * 2);
                        } else {
                            System.out.println("No ganaste nadota :C");
                        }
                    }
                }
                default -> {
